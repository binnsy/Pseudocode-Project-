...
def ask_if_continue():
    ...
    playing = raw_input("Would you like to start an adventure?(type exit to exit) ")
    if playing == "exit":
        exit()
    return playing 
...
def Obstacle_five_TT():
    ...
    if Obstacle_five_TT == "run":
        print "The snow is so thick and the bear is catching up. There is no way you can outrun the bear. He catched up and attacks. So close, yet so far from the top. Too injured to continue on. You will not make it today."
...
def Obstacle_three_SS():
    ...
    Obstacle_three_SS = raw_input("Even though you hate mushrooms, do you eat the mushrooms or take a risk on the rasberries?")
